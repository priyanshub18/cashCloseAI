...
test("ships the product asset and removes starter preview code", async () => {
  ...
  assert.match(app, /5-minute showcase/);
  ...
  assert.match(app, /Two bounded Responses turns, preserved after the run/);
  assert.match(app, /In-memory API state \(demo\)/);
  assert.match(app, /No probability band generated/);
